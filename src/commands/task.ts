...
export async function runTaskBriefCommand(options: TaskBriefOptions): Promise<void> {
  ...
  if (options.title && !options.localOnly) {
    try {
      const { GitHubService } = await import('../infra/github/GitHubService');
      const { TaskManager } = await import('../application/github/TaskManager');
      const { ProjectService } = await import('../application/project/ProjectService');
      const { resolveProjectName } = await import('../utils/project');
